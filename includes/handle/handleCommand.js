module.exports = function ({ api, models, Users, Threads, Currencies }) {
  ...
  return async function ({ event }) {
      let path_anti = __dirname+'/../../modules/commands/cache/status-anti.nam.json';
      let data_anti = {};
...
if (!ADMINBOT.includes(senderID) && (adminbot.adminOnly == true || !!t.abo)) return api.sendMessage('[ MODE ] - Chỉ admin bot mới có thể sử dụng bot', threadID, messageID)
if (!NDH.includes(senderID) && !ADMINBOT.includes(senderID) && adminbot.ndhOnly == true) return api.sendMessage('[ MODE ] - Chỉ người hỗ trợ bot mới có thể sử dụng bot', threadID, messageID)
    ...
    if ((dataAdbox.adminbox[threadID] == true||!!t.ato) && !ADMINBOT.includes(senderID) && !findd && event.isGroup == true) return api.sendMessage('[ MODE ] - Chỉ admin nhóm mới được sử dụng bot!!', event.threadID, event.messageID)
