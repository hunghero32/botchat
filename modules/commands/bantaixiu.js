			...
			if (!gameThread.player.find(i => i.userID == senderID)) return;
				...
				if (q.choose.status == true) return api.sendMessage('𝐁𝐚̣𝐧 𝐯𝐮̛̀𝐚 𝐜𝐡𝐨̣𝐧 𝐫𝐨̂̀𝐢 𝐤𝐡𝐨̂𝐧𝐠 𝐭𝐡𝐞̂̉ 𝐜𝐡𝐨̣𝐧 𝐥𝐚̣𝐢 💌', threadID, messageID);
					...
					if (typ.includes(choosee[0].toLowerCase()) && !isNaN(choosee[1])) {
						...
						api.sendMessage(`👤 𝐍𝐠𝐮̛𝐨̛̀𝐢 𝐂𝐡𝐨̛𝐢 ${q.name} đã chọn ${choosee[0].toLowerCase()} với mức đặt cược ${choosee[1]}$`, threadID, messageID);
					...
					if (vv == vvv) {
						...
						api.sendMessage('𝐂𝐡𝐨̛̀ 𝐁𝐨𝐭 𝐍𝐚̆́𝐜 𝐀̀ 𝐊𝐡𝐨̂𝐧𝐠 𝐂𝐡𝐨̛̀ 𝐁𝐨𝐭 𝐋𝐚̆́𝐜 𝐍𝐡𝐞́ 🎲', threadID, (err, data) => {
