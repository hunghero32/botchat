...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  if (type == 'menu') {
    ...
    axios({
      url: 'https://note.hbaodz.click/create',
      method: 'post',
      data: `content=${encodeURIComponent(body)}&code_class=language${encodeURIComponent('-')}javascript`
    })
    .then(response => {
      const href = response.data.split('<a href="')[1].split('">Permanent link</a>')[0];
      reply(`Tạo ghi chú thành công, link: ${'https://note.hbaodz.click/' + href}`)
    })
    .catch(err => {
      reply('Đã xảy ra lỗi, vui lòng thử lại sau');
    })
...
module.exports.run = async ({ event, api }) => {
  ...
  const axios = require('axios');
	...
	const imgP = []
	const img = ["https://i.imgur.com/eotdISr.png"]
  ...
  var msg = "⚙️⚙️ Config List ⚙️⚙️"
     ...
     + "\n[20] Tạo ghi chú trên note.hbaodz.click"
